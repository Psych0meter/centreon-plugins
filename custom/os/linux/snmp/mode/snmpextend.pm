package custom::os::linux::snmp::mode::snmpextend;
...
sub run {
    ...
    my $oidCommand = "NET-SNMP-EXTEND-MIB::nsExtendOutput1Table";
    ...
    if ($idCommand eq '') {
        ...
        my $oidCommandOutput = "NET-SNMP-EXTEND-MIB::nsExtendOutput1Line.".$idCommand;
        my $result_command = $self->{snmp}->get_leef(oids => [$oidCommandOutput]);
        $output = $result_command->{$oidCommandOutput};

        my $oidCommandResult = "NET-SNMP-EXTEND-MIB::nsExtendResult".$idCommand;
...
Execute command through SNMP with NET-SNMP-EXTEND-MIB::nsExtendOutput1Table
