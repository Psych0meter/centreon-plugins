package custom::database::mysql::mode::replicationmastermaster;
...
sub check_replication {
    ...
    if ($self->{$master->get_id()}->{exit} == -1) {
		...
		} elsif ($num_sec_lates >= $self->{option_results}->{critical}) {
			$position_status = -2;
			$position_status_error .= " Slave replication is $num_sec_lates seconds behind master.";
		} elsif ($num_sec_lates >= $self->{option_results}->{warning}) {
			$position_status = -1;
			$position_status_error .= " Slave replication is $num_sec_lates seconds behind master.";
		}
    }
