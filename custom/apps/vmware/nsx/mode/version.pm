package custom::apps::vmware::nsx::mode::version;
...
sub run {
    ...
    my $url = 'api/1.0/appliance-management/summary/system';

    my $xml = `curl -u $username:$password https://$hostname/$url -k -s`;

    my $data = XMLin($xml);

    my $version = $data->{versionInfo}->{majorVersion}.".".$data->{versionInfo}->{minorVersion}.".".$data->{versionInfo}->{patchVersion}.".".$data->{versionInfo}->{buildNumber};

    $self->{output}->output_add(severity => "OK",
                                short_msg => sprintf("Version : %s", $version));
