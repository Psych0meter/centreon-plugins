package custom::apps::vmware::nsx::mode::uptime;
...
sub run {
    ...
    my $url = 'api/1.0/appliance-management/system/uptime';

    my $output = `curl -u $username:$password https://$hostname/$url -k -s`;

    $self->{output}->output_add(severity => "OK",
                                short_msg => sprintf("Uptime : %s", $output));
