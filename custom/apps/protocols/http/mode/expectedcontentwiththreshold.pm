package custom::apps::protocols::http::mode::expectedcontentwiththreshold;
...
sub new {
    ...
    $options{options}->add_options(arguments =>
            ...
            "warning-expected-string:s"		=> { name => 'warning_expected_string' },
			"critical-expected-string:s"	=> { name => 'critical_expected_string' },
...
sub check_options {
    ...
    if (!defined($self->{option_results}->{critical_expected_string})) {
        $self->{output}->add_option_msg(short_msg => "You need to specify --critical-expected-string option.");
        $self->{output}->option_exit();
    }
    if (!defined($self->{option_results}->{warning_expected_string})) {
        $self->{output}->add_option_msg(short_msg => "You need to specify --warning-expected-string option.");
        $self->{output}->option_exit();
    }
...
sub run {
    ...
    if ($webcontent =~ /$self->{option_results}->{warning_expected_string}/mi) {
        $self->{output}->output_add(severity => 'WARNING',
                                    short_msg => sprintf("'%s' is present in content.", $webcontent));
    } elsif ($webcontent =~ /$self->{option_results}->{critical_expected_string}/mi) {
...
=item B<--warning-expected-string>

Specify WARNING String to check on the Webpage

=item B<--critical-expected-string>

Specify CRITICAL String to check on the Webpage
